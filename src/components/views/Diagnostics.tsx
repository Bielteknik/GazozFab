      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 flex-1 min-h-0">
        ...
        <div className="bg-[#151921] border border-[#2D333F] rounded p-3 overflow-y-auto">
            <h3 className="text-[10px] uppercase font-bold text-gray-400 mb-3 border-l-2 border-[#F97316] pl-2 flex justify-between">
              Denetleyici Bağlantı Durumları
           </h3>
           <div className="space-y-1.5 font-mono text-[10px]">
              <div className="flex justify-between items-center text-gray-300">
                <span>Ana Kontrolcü (Raspberry Pi 5)</span>
                <span className="text-green-400 font-bold">ÇEVRİMİÇİ</span>
              </div>
              {data.nanos.map(nano => (
                <div key={nano.id} className="flex justify-between items-center text-gray-300">
                  <span>{nano.name} ({nano.id})</span>
                  <span className={cn(
                    "font-bold",
                    nano.status === 'ONLINE' ? 'text-green-400' : 'text-red-500'
                  )}>
                    {nano.status === 'ONLINE' ? 'BAĞLI' : 'BAĞLANTI YOK'}
                  </span>
                </div>
              ))}
              {data.nanos.length === 0 && (
                <div className="text-gray-500 text-center py-2">Yapılandırılmış Nano Denetleyici Yok</div>
              )}
