...
export function History({ data }: { data: SystemData }) {
  ...
  const avgDuration = useMemo(() => {
    if (data.cycleHistory.length === 0) return '0.0s';
    const total = data.cycleHistory.reduce((acc, c) => acc + c.duration, 0);
    return (total / data.cycleHistory.length / 1000).toFixed(1) + 's';
  }, [data.cycleHistory]);
        ...
        <div className="col-span-1 md:col-span-8 flex flex-col bg-[#151921] border border-[#2D333F] rounded overflow-hidden p-3">
           ...
           <div className="mt-4 grid grid-cols-3 gap-3 border-t border-[#374151] pt-4">
              <div>
                 ...
                 <div className="text-lg font-mono text-gray-200 mt-0.5">{avgDuration}</div>
              </div>
              <div>
