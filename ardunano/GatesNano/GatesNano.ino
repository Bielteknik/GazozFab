...
int Y_DIR = 3;
int ENABLE_PIN = 8;
int SENSOR_X = 14; // Giriş Lazer Sensörü (A0 = D14)
int SENSOR_Y = 15; // Çıkış Lazer Sensörü (A1 = D15)
...
int sensYState = HIGH;
int limitInState = LOW;  // D9 Optik Switch (Giriş Kilidi)
int limitOutState = LOW; // D10 Optik Switch (Çıkış Kilidi)
int lastReadingX = HIGH; // Son okunan anlık değer
int lastReadingY = HIGH;
int lastReadingLimitIn = LOW;
int lastReadingLimitOut = LOW;
unsigned long lastDebounceTimeX = 0;
unsigned long lastDebounceTimeY = 0;
unsigned long lastDebounceTimeLimitIn = 0;
unsigned long lastDebounceTimeLimitOut = 0;
unsigned long debounceDelay = 50; // milisaniye

int LIMIT_IN_PIN =
    9; // Giriş Kapısı Optik Limit Switch (D9 - CNC Shield X-Limit)
int LIMIT_OUT_PIN =
    10; // Çıkış Kapısı Optik Limit Switch (D10 - CNC Shield Y-Limit)

// --- PCINT (Donanımsal Kesme) Değişkenleri ---
// İPTAL EDİLDİ: Sadece tam şişeyi saymak için "Kalıcı Tarama" (Polling)
// yöntemine geçildi.
...
void setup() {
  ...
  limitOutState = digitalRead(LIMIT_OUT_PIN);
  lastReadingX = sensXState;
  lastReadingY = sensYState;
  lastReadingLimitIn = limitInState;
  lastReadingLimitOut = limitOutState;
...
void checkSensors() {
  unsigned long laserDebounce =
      30; // 30ms stabil bekleme: Kısa kesintileri (kıyı köşe) reddet, sadece
          // tam şişeyi say.

  // 1. Lazer Sensör X (Giriş) Kalıcı Filtreli Tarama
  int readingX = digitalRead(SENSOR_X);
  if (readingX != lastReadingX) {
    lastDebounceTimeX = millis(); // Değişim anında sayacı sıfırla
  }
  if ((millis() - lastDebounceTimeX) > laserDebounce) {
    // 30ms boyunca lazer kesintisi BOZULMADAN devam ettiyse bu gerçek bir
    // şişedir
  ...
  // 2. Lazer Sensör Y (Çıkış) Kalıcı Filtreli Tarama
  int readingY = digitalRead(SENSOR_Y);
  if (readingY != lastReadingY) {
    lastDebounceTimeY = millis();
  }
  if ((millis() - lastDebounceTimeY) > laserDebounce) {
  ...
  // 2.5 Optik Limit Switch (Giriş Kapısı - D9) Takibi
  int readingLimitIn = digitalRead(LIMIT_IN_PIN);
  if (readingLimitIn != lastReadingLimitIn) {
    lastDebounceTimeLimitIn = millis();
  }
  if ((millis() - lastDebounceTimeLimitIn) > debounceDelay) {
    if (readingLimitIn != limitInState) {
      limitInState = readingLimitIn;
  ...
  lastReadingLimitIn = readingLimitIn;

  // 2.6 Optik Limit Switch (Çıkış Kapısı - D10) Takibi
  int readingLimitOut = digitalRead(LIMIT_OUT_PIN);
  if (readingLimitOut != lastReadingLimitOut) {
    lastDebounceTimeLimitOut = millis();
  }
  if ((millis() - lastDebounceTimeLimitOut) > debounceDelay) {
    if (readingLimitOut != limitOutState) {
      limitOutState = readingLimitOut;
      if (limitOutState == HIGH) {
  ...
  lastReadingLimitOut = readingLimitOut;
}
...
void motorStep(int stepPin, int dirPin, bool yon, long adim) {
  ...
  for (long i = 0; i < adim; i++) {
    ...
    // Motor dönerken bloklanmamak ve şişe kaçırmamak için lazerleri sürekli
    // kontrol et
    checkSensors();
  }
...
void processCommand(String cmd) {
  if (cmd.startsWith("CONFIG:")) {
    ...
    if (pin >= 2 && pin <= 13) {
      ...
      if (pin != ENABLE_PIN && pin != X_STEP && pin != X_DIR && pin != Y_STEP &&
          pin != Y_DIR && pin != SENSOR_X && pin != SENSOR_Y) {
        pinMode(pin, OUTPUT);
